...
const Home = () => {
    ...
    const validateLoginDetails = (loginEmail, loginPass) =>{
        if(user){
            user.map(item=>{
                const {email, pass, team, fullName} = item;
    ...
    const validLogin = async ({ email, password }) => {
        let response = await fetch(`https://my-json-server.typicode.com/pritam-shinde/task-api/profile/?email=${email}`, {
            method:"GET",
            headers: {
    ...
    if(localStorage.getItem("email") && localStorage.getItem("pass") && localStorage.getItem("team")){
        return <Dashboard team={localStorage.getItem("team")} />
    }
                                ...
                                <Grid container>
                                    ...
                                    <Grid item xs={12} lg={4}>
                                        <Box className="shadow-lg" component={Paper} py={8}>
                                            <Box>
                                                <Typography align="center" className={`text-uppercase ${classes.heading}`} variant="h1">sign in <span style={{ color: "#47d7ac", fontWeight: 700 }}>for free</span></Typography>
                                            </Box>
                                            <Box pt={5} px={6}>
